...
export class Gd1Service {
  ...
  saveStudentSpecialization(payload: Gd1RegistrationCreateRequest): Observable<Gd1RegistrationApiResponse> {
        ...
        subtitle:
          stage === 1
            ...
            : stage === 2
              ? 'Đăng ký GVHD'
              : 'Thực hiện Đồ án',
        badge: this.getStageBadge(period?.status ?? null, stage, isCompleted, isActive),
        badgeClass: this.getStageBadgeClass(period?.status ?? null, stage, isCompleted, isActive),
  ...
  private getTheme(index: number): {
    color: Specialization['color'];
    icon: Specialization['icon'];
    accentIcon: Specialization['accentIcon'];
  } {
    const themes = [
